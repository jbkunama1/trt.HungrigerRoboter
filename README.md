# 🤖 Hungriger Roboter (Hungry Robot)

<a href="https://www.buymeacoffee.com/highfish">
<img src="https://cdn.buymeacoffee.com/buttons/default-orange.png" alt="Buy Me A Coffee" height="41" width="174">
</a>

[![Made for Education](https://img.shields.io/badge/made%20for-education-blue)](#)
![Bildungsplan 2016](https://img.shields.io/badge/Bildungsplan-2016%20BW-orange)
![Klasse 9](https://img.shields.io/badge/Klasse-9-yellow)

> 🎯 **Zusatzaufgabe Klasse 9 – Gemeinschaftsschule BW (Bildungsplan 2016)**  
> Thema der Einheit: **Arduino Uno und Sensoren (+Aktoren)**  
> Projekt: **Hungriger Roboter / Hungry Robot** – auf Basis von Arduino, HC‑SR04 und Servo.

---

## Struktur des Repos

```text
trt.HungrigerRoboter
├── README.md
├── index.md
├── code/
│   └── HungrigerRoboter_HCSR04.ino
├── arbeitsblaetter/
│   ├── AB0_Deckblatt.md
│   ├── AB1_Anleitung_HungrigerRoboter.md
│   ├── AB2_Tinkercad_Programmierung.md
│   ├── AB3_Schaltplan_Verdrahtung.md
│   ├── AB4_Differenzierung_Erweiterung.md
│   └── pdfs/
└── unterricht/
    ├── Ablaufplan_Stunde_Arduino_HCSR04_HungrigerRoboter.md
    └── Didaktik_Bildungsplan2016_Technik_GMS9.md
```

## Hinweise

- Die 5 beigefügten PDFs liegen im Ordner `arbeitsblaetter/pdfs/`.
- Die Arbeitsblätter sind als gestufte Hilfen aufgebaut: AB1 → AB2 → AB3 → AB4.
- `index.md` kann als einfache GitHub-Pages-Startseite genutzt werden.

